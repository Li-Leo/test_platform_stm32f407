...
int flex_button_main(void)
{
    ...
    keyTaskHandle = osThreadNew(button_scan, NULL, &key_task_attributes);

    return 0;
}

static void common_btn_evt_cb1(void *arg)
{
    flex_button_t *btn = (flex_button_t *)arg;

    printf("id: [%d - %s]  event: [%d - %30s]  repeat: %d button1-------\r\n", 
        btn->id, enum_btn_id_string[btn->id],
        btn->event, enum_event_string[btn->event],
        btn->click_cnt);
...
void button1(void)
{
    user_button[USER_BUTTON_1].cb = common_btn_evt_cb1;
}

void button10(void)
{
    user_button[USER_BUTTON_1].cb = common_btn_evt_cb;
}

SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), button1, button1, button1);
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), button0, button10, button0);
