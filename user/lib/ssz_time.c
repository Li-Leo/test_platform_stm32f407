...
static DateTimeStruct g_time_last_read_str_time;
static DateTimeStruct g_time_last_read_str_time2;
/************************************************
* Variable 
************************************************/
...
const char* ssz_time_str(const DateTimeStruct *in_time, char buff[SSZ_TIME_STR_SIZE]) {
	const DateTimeStruct* p;
	if (in_time == NULL) {
		p = ssz_time_now();
	}
	else {
...
*/
int ssz_time_format(char* str, int str_size, const char* format, const DateTimeStruct *in_time) {
	struct tm time2;
