...
#include "Lib/ssz_time.h"
#include "Lib/ssz_str_utility.h"
#include "ssz_check.h"
...
int32_t ssz_time_to_seconds(const DateTimeStruct *time){
	return ssz_time_to_seconds_ex(time, 0);
}

//the start time is SSZ_BASE_YEAR.1.1 00:00:00, like mktime
int32_t ssz_time_to_seconds_ex(const DateTimeStruct *time, int time_zone)
{
    int t;
    int32_t seccount = 0;
...
void ssz_seconds_to_time(int32_t seconds, DateTimeStruct *time){
	ssz_seconds_to_time_ex(seconds, time, 0);
}

//the start time is SSZ_BASE_YEAR.1.1 00:00:00 , like gmtime or localtime
void ssz_seconds_to_time_ex(int32_t seconds, DateTimeStruct *time, int time_zone)
{
	seconds += time_zone*3600;
    int32_t days = seconds / (24 * 3600);
    int32_t seconds_at_one_day = seconds % (24 * 3600);

    memset(time, 0, sizeof(DateTimeStruct));
...
DateStruct ssz_str_to_date(const char* date_str, SszDateFormat date_format)
{
    DateStruct date;
	int nums[3];
...
TimeStruct ssz_str_to_time(const char* time_str)
{
    TimeStruct time1;
	int nums[3];
